...
def create_returns_tear_sheet(factor_data,
    ...
    if by_group:
        ...
        # gf.savefig(gl_dict["PATH_PLOT"] + "quantile_returns_bar.png")
        gf.close()
...
def create_information_tear_sheet(factor_data,
    ...
    plotting.plot_ic_ts(ic, ax=None) #ax=ax_ic_ts)

    ax_ic_hqq = [gf.next_cell() for _ in range(fr_cols * 2)]
    plotting.plot_ic_hist(ic, ax=None) #ax=ax_ic_hqq[::2])
    plotting.plot_ic_qq(ic, ax=None) #ax=ax_ic_hqq[1::2])
    ...
    if by_group:
        ...
        plotting.plot_ic_by_group(mean_group_ic, ax=None) #ax=gf.next_row())

    # 修改
    gf.fig.savefig(gl_dict["PATH_PLOT"] + "ic_info.png")
    # 修改结束
