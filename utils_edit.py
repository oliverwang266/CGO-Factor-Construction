...
def quantize_factor(factor_data,
    ...
    def quantile_calc(x, _quantiles, _bins, _zero_aware, _no_raise):
        ...
        if x.index[0][0] == datetime.strptime("2010-03-29","%Y-%m-%d"):
            a = 0
        try:
...
def compute_forward_returns(factor,
    ...
    # MultiIndex.set_names(names=["date","asset"], level=0)
    ...
    merged_data : pd.DataFrame - MultiIndex
        ...
        ::
           ...
           2014-01-01 | CMG   | 0.03| 0.09| 0.036|  1.7 |  G2 |      1
                      ...
                      --------------------------------------------------------
    """
    if gl_dict["RTN_EXIST"] == False:
        forward_returns = compute_forward_returns(factor, prices, periods, filter_zscore, cumulative_returns)

        # # 完美预测下的特征分析
        # factor["factor_value"] = forward_returns["5D"]
        ...
        forward_returns.reset_index(drop=False).to_feather(gl_dict["PATH_CACHE"] + "forward_returns.feather")
        factor_data.reset_index(drop=False).to_feather(gl_dict["PATH_CACHE"] + "factor_data.feather")
    else:
        # forward_returns = pd.read_feather(path_cache + "forward_returns.feather").set_index(["date", "asset"])
        factor_data = pd.read_feather(gl_dict["PATH_CACHE"] + "factor_data.feather").set_index(["date", "asset"])
        # from pandas.tseries.offsets import CustomBusinessDay, Day, BusinessDay
        factor_data.index.levels[0].freq = CustomBusinessDay()
